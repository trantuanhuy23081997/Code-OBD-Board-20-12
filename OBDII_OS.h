...
static void Task_SetupOBD(void* pvParameters)
{
  #if DEBUG 
    if ( xSemaphoreTake( xSerialSemaphore, ( TickType_t ) 5 ) == pdTRUE ) {
      Serial.println("Task Setup OBD running and Delete Task Setup A9G");  //  Đoạn code thêm vào để kiểm tra
      xSemaphoreGive( xSerialSemaphore ); 
    };
  #endif 
  BaseType_t qStatus;
  BaseType_t qStatus_DTC;
  BaseType_t qStatus_Data;
        ...
        if (qStatus  == pdPASS){
           ...
           if (strstr(Rx, "0343 00")   == &Rx[0] || (strstr(Rx, "03")   == &Rx[0] &&strstr(Rx, "43")   == &Rx[9])){ 
              ...
              if (qStatus_DTC !=  pdPASS){
                #if DEBUG
                if ( xSemaphoreTake( xSerialSemaphore, ( TickType_t ) 5 ) == pdTRUE ){
                   Serial.println(F("Could not send to the OBDQueue_DTC"));
                ...
                #if DEBUG
                  if ( xSemaphoreTake( xSerialSemaphore, ( TickType_t ) 5 ) == pdTRUE ){
                    Serial.println(F("Sended data to the OBDQueue_DTC successfully"));
                    xSemaphoreGive( xSerialSemaphore ); // Now free or "Give" the Serial Port for others.
                  };
                #endif
              }        
          ...
          if (Cmplete_Sent == 6){
            qStatus_Data = xQueueSend(OBDQueue_Data, &DataOBD2, ( TickType_t ) 100);
            if (qStatus_Data == pdPASS){
              #if DEBUG 
                if ( xSemaphoreTake( xSerialSemaphore, ( TickType_t ) 5 ) == pdTRUE ) {
                  Serial.println("Sended data to OBDQueue_Data successfully");  //  Đoạn code thêm vào để kiểm tra
                  xSemaphoreGive( xSerialSemaphore ); 
                };
              #endif
            }
              ...
              #if DEBUG 
                if ( xSemaphoreTake( xSerialSemaphore, ( TickType_t ) 5 ) == pdTRUE ) {
                  Serial.println("Could not send data to OBDQueue_Data");  //  Đoạn code thêm vào để kiểm tra
                  xSemaphoreGive( xSerialSemaphore ); 
                };
              #endif
            }
